...
import 'package:hys/SocialPart/Podcast/services/audio_player_service.dart';
...
class AudioPlayerController with ChangeNotifier {
  ...
  AudioPlayerController(String path) {
    ...
    _playerStateChangedSubscription = _audioPlayerService.playerStateStream.listen((state) {
      logger.d('player state changed: $state');
      notifyListeners();
    });
    _durationChangedSubscription =_audioPlayerService.playerDurationStream.listen((duration) {
  ...
  double getVolume()  {
    return _audioPlayerService.getVolume();
  }
