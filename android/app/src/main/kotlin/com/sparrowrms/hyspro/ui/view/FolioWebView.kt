...
import com.sparrowrms.hyspro.model.HighlightImpl.HighlightStyle
...
import com.sparrowrms.hyspro.ui.activity.FolioActivityCallback
import com.sparrowrms.hyspro.ui.activity.UserListActivity
import com.sparrowrms.hyspro.ui.fragment.DictionaryFragment
import com.sparrowrms.hyspro.ui.fragment.FolioPageFragment
import com.sparrowrms.hyspro.util.AppUtil
import com.sparrowrms.hyspro.util.HighlightUtil
...
class FolioWebView : WebView {
    ...
    fun initViewTextSelection() {
        ...
        val type= SharedPrefsHelper.getOpenBookType()
        ...
        if(type=="1"){
            viewTextSelection.predictConceptContainer.visibility= GONE
        }else if(type=="2"){
            viewTextSelection.predictConceptContainer.visibility= VISIBLE
            viewTextSelection.highlyRatedBooksContainer.visibility= GONE
            viewTextSelection.relatedQuestions.visibility= GONE
        }
        ...
        viewTextSelection.highlyRatedBooks.setOnClickListener {
            dismissPopupWindow()
            loadUrl("javascript:onTextSelectionItemClicked(${it.id})")
        }
        viewTextSelection.predictConceptButton.setOnClickListener {
            dismissPopupWindow()
            loadUrl("javascript:onTextSelectionItemClicked(${it.id})")
        }
        viewTextSelection.relatedQuestionButton.setOnClickListener {
            dismissPopupWindow()
            loadUrl("javascript:onTextSelectionItemClicked(${it.id})")
        }
        viewTextSelection.audioCall.setOnClickListener {
            dismissPopupWindow()
            withMultiChoiceList()
            //  loadUrl("javascript:onTextSelectionItemClicked(${it.id})")
        }
        viewTextSelection.toughNessLevel.setOnClickListener {
            dismissPopupWindow()
            toughNessTypeDialog()

        }
        viewTextSelection.examLikeHood.setOnClickListener {
            dismissPopupWindow()
            examLikeHoodTypeDialog()
        }
...
//        val items = arrayOf("Audio", "Video", "ScreenShare")
//
//        val builder = AlertDialog.Builder(context)
//
//        builder.setTitle("Select an option")
//        builder.setItems(items, DialogInterface.OnClickListener{ _, _ ->
...
//        builder.show()
    ...
    private fun toughNessTypeDialog() {

        val items = arrayOf("High", "Medium", "Low")
        val builder = AlertDialog.Builder(context)
        builder.setTitle("Select Difficulty Level")
        builder.setItems(items, DialogInterface.OnClickListener { dialog, index ->
            if (index == 0) {
                onToughnessSelected(ToughnessImpl.ToughNessType.High, false)
            } else if (index == 1) {
                onToughnessSelected(ToughnessImpl.ToughNessType.Medium, false)
            } else {
                onToughnessSelected(ToughnessImpl.ToughNessType.Low, false)
            }
        })
        builder.create()
    ...
    private fun examLikeHoodTypeDialog() {

        val items = arrayOf("High", "Medium", "Low")
        val builder = AlertDialog.Builder(context)
        builder.setTitle("Select ExamLikeHood Type")
        builder.setItems(items, DialogInterface.OnClickListener { _, index ->
            when (index) {
                0 -> {
                    onExamLikeHoodSelected(ExamLikeHoodImpl.ExamLikeHoodType.High, false)
                }
                1 -> {
                    onExamLikeHoodSelected(ExamLikeHoodImpl.ExamLikeHoodType.Medium, false)
                }
                else -> {
                    onExamLikeHoodSelected(ExamLikeHoodImpl.ExamLikeHoodType.Low, false)
                }
            }
        })
        builder.create()
    ...
    fun onTextSelectionItemClicked(id: Int, selectedText: String?) {
        ...
        when (id) {
            ...
            R.id.highlyRatedBooks -> {
                goToHighlyRelatedBooks(selectedText)
            }
            R.id.relatedQuestionButton ->{
                goToHighlyRelatedQuestion(selectedText)
            }
            R.id.predictConceptButton ->{
                goToPredictedConcept(selectedText)
            }
            else -> {
    ...
    private fun goToHighlyRelatedBooks(selectedText: String?) {
        folioActivityCallback.gotoHighRatedBooks(selectedText)

    }
    private fun goToHighlyRelatedQuestion(selectedText: String?) {
        folioActivityCallback.goToRelatedQuestion(selectedText)

    }
    private fun goToPredictedConcept(selectedText: String?) {
        folioActivityCallback.predictChapterCallback(selectedText)
    ...
    private fun onToughnessSelected(style: ToughnessImpl.ToughNessType, isAlreadyCreated: Boolean) {
        parentFragment.toughnessSelected(style, isAlreadyCreated)
        dismissPopupWindow()
    }

    private fun onExamLikeHoodSelected(style: ExamLikeHoodImpl.ExamLikeHoodType, isAlreadyCreated: Boolean) {
        parentFragment.examLikeHoodSelected(style, isAlreadyCreated)
        dismissPopupWindow()
    }
