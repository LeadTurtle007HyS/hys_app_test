...
import com.sparrowrms.hyspro.ui.view.UnderlinedTextView;
import com.sparrowrms.hyspro.util.AppUtil;
import com.sparrowrms.hyspro.util.UiUtil;

import java.util.List;

public class ExamLikeHoodAdapter extends RecyclerView.Adapter<ExamLikeHoodAdapter.HighlightHolder> {
    private List<ExamLikeHoodImpl> highlights;
    private ExamLikeHoodAdapterCallback callback;
    private Context context;
    private Config config;

    public ExamLikeHoodAdapter(Context context, List<ExamLikeHoodImpl> highlights, ExamLikeHoodAdapterCallback callback, Config config) {
    ...
    public HighlightHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new HighlightHolder(LayoutInflater.from(parent.getContext())
                .inflate(R.layout.difficulty_level_row, parent, false));
    }

    @Override
    public void onBindViewHolder(final HighlightHolder holder, final int position) {

        holder.container.postDelayed(() -> ((AppCompatActivity) context).runOnUiThread(() -> holder.container.setLayoutParams(new FrameLayout.LayoutParams(FrameLayout.LayoutParams.WRAP_CONTENT, FrameLayout.LayoutParams.WRAP_CONTENT))), 10);
        ...
        if(getItem(position).getType().equalsIgnoreCase("toughness_high")){
            holder.difficultyType.setText(R.string.difficulty_level_high);
        }else if(getItem(position).getType().equalsIgnoreCase("toughness_medium")){
            holder.difficultyType.setText(R.string.difficulty_level_medium);
        }else {
            holder.difficultyType.setText(R.string.difficulty_level_low);
        }
    ...
    private ExamLikeHoodImpl getItem(int position) {
    ...
    static class HighlightHolder extends RecyclerView.ViewHolder {
        ...
        private TextView date,difficultyType;
        ...
        HighlightHolder(View itemView) {
            ...
            difficultyType=itemView.findViewById(R.id.difficultyType);
        }
    }

    public interface ExamLikeHoodAdapterCallback {
        void onItemClick(ExamLikeHoodImpl toughnessImpl);

        void deleteHighlight(int id);

        void editNote(ExamLikeHoodImpl highlightImpl, int position);
    }
}
