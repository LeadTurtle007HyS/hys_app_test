...
$(function () {
    window.ssReader = Class({
        ...
        init: function () {
            ...
            this.highlighter.addClassApplier(rangy.createClassApplier("toughness_high", {
                          ignoreWhiteSpace: true,
                          tagNames: ["span", "a"]
              }));

            this.highlighter.addClassApplier(rangy.createClassApplier("toughness_medium", {
                          ignoreWhiteSpace: true,
                          tagNames: ["span", "a"]
            }));

            this.highlighter.addClassApplier(rangy.createClassApplier("toughness_low", {
                          ignoreWhiteSpace: true,
                          tagNames: ["span", "a"]
            }));
        ...
        highlightSelection: function (color) {
            try {
                 console.log("highlightSelection1 : "+color);
          ...
          toughnessSelection: function (type) {
                    try {

                        this.highlighter.toughnessSelection(type, null);
                        var range = window.getSelection().toString();
                        var params = {content: range, rangy: this.getHighlights(), color: type};
                        this.clearSelection();
                        Highlight.onReceiveToughNess(JSON.stringify(params));
                    } catch (err) {
                        console.log("highlightSelection : " + err);
                    }
                },
          examLikeHoodSelection: function (type) {
                    try {
                         console.log("highlightSelection1 : "+type);
                        this.highlighter.examLikeHoodSelection(type, null);
                        var range = window.getSelection().toString();
                         console.log("highlightSelection2 : "+range);
                        var params = {content: range, rangy: this.getHighlights(), color: type};
                         console.log("highlightSelection3 : "+type);
                        this.clearSelection();
                        Highlight.onReceiveExamLikeHood(JSON.stringify(params));
                    } catch (err) {
                        console.log("highlightSelection : " + err);
                    }
                },
